...
def _resolve_stakeholder_profile(stakeholder_id: str | None, stakeholder_profile: str | None) -> str:
    ...
    if not stakeholder_id:
        raise HTTPException(
            status_code=400,
            detail="Provide either stakeholder_profile or stakeholder_id.",
        )
...
def _run_simulation(runtime: SimulationRuntime, request: StartSimulationRequest, stakeholder_profile: str) -> None:
    try:
        graph = build_graph()
...
def start_simulation(request: StartSimulationRequest) -> StartSimulationResponse:
    ...
    worker = threading.Thread(
        ...
        args=(runtime, request, stakeholder_profile),
